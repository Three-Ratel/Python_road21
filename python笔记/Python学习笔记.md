...
6. **线程具有以下属性**
   ...
   2. 线程的实体包括程序、数据和TCB。线程是动态概念，它的动态特性由线程控制块TCB（Thread Control Block）描述。
      - 线程状态。
      - 当线程不运行时，被保存的现场资源。
      - 一组执行堆栈。
      - 存放每个线程的局部变量主存区。
...
1. 在一个基于协程的应用程序中，可能会产生数以千记的协程，所有这些协程，会有一个的**调度器来统一调度**。
...
5. CPU**只认识线程**，不会像线程一样吧上下文保存在CPU寄存器，协程是用户控制的。
6. 协程的**优缺点**
   1. 协程的好处
