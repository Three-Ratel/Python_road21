...
2. **交换机(4)**
   ...
   - **保留网段（私有IP）**：192.168.0.0-192.168.255.255 /172.16.0.0-172.31.255.255 /10.0.0.0-10.255.255.255
...
1.  **所有的应用层协议，需要区分是基于TCP还是UDP协议的**
2.  **CDN**：基于**UDP**协议，请求的静态文件(**内容分发**)，帮主用户加速访问速度
...
2. **网络协议**
   ...
   - **传输层**协议:TCP协议、UDP协议。 
   - **应用层**协议:FTP、Telnet、SMTP、HTTP、RIP、NFS、DNS、POP3、TFTP
...
2. socket历史
   ...
   3. **基于网络类型的套接字家族**
      - 套接字家族的名字：**AF_INET**，(还有**AF_INET6**被用于ipv6，还有一些其他的地址家族，不过，他们要么是只用于某个平台，要么就是已经被废弃，或者是很少被使用，或者是根本没有实现，所有地址家族中，AF_INET是使用最广泛的一个，python支持很多种地址家族，但是由于我们只关心网络编程，所以大部分时候我们只使用AF_INET)
...
1.  阻塞io模型：accept、recv 阻塞 
2.  非阻塞io模型：accept、recv 不阻塞 
3.  io多路复用
4.  异步io
5.  时间驱动模型
...
rlist = [sk]
while True:
    rli, wli, eli = select.select(rlist, [], [])
	for i in rli:
        if i is sk:
    		conn, _ = i.accpet()
    		rlist.append(conn)
...
-   os帮助程序，对所有监听对象进行轮询，一旦有数据来，就会把对应的对象返回
-   缺点：
    1.  由于底层数据结构的问题，select能够代理的网络对象是有限的
    2.  把一对SK，CONN对象交给操作系统处理，是在所有操作系统都有的，操作系统采用轮询的方式（**标识符问来的数据是谁的**）,随着监听的对象越来越多，效率会越来越差，轮询的时间会延迟，能够处理的监听个数是有限的，超过一定的个数（**1024个连接**），就不能进行监听了

## 4. poll机制

-   使用轮询
-   对select机制的底层数据结构进行优化，能够代理的网络对象没有限制

## 5. epoll

-   Windows上没有，Linux上有，**不用轮询，采用回调函数的机制**，一旦数据来了，立即去调用一个函数让这个函数去通知是哪个对象的数据来了。
-   无论监听多少网络对象，效率都是一样的。
