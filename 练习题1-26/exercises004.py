...
2. li.extend(s1) # ,遍历 s1 中的每个元素，追加到list中
3. li.insert(index, 'a') #insert 只能识别单个索引值
4. li.pop(index) # 可以获取删除值
5. li.remove('a') # 删除指定元素，li中没有会报错
6. ','.join(li) # 只要支持循环就支持 join，操作对象必须是 str 否则报错
7. s.isdigit() # 有空格和其他字符都会返回  False
'''
###############################  end  #####################################
...
# li = ["alex", "WuSir", "ritian", "barry", "wenzhou"]
# li.pop(1)  # 此时可以输出删除元素 li = li.pop(1)
# print(li)
#
